	...
	module.exports = function(init, app, db) {
	    ...
	    app.get(backendDirectoryPath + "/cv/:filename", (req, res) => {
	        var db = new mongodb.Db(init.dbName, new mongodb.Server(init.mongoHost, init.mongoPort));
	        // IMPROVEMENTS
	        // PROMISIFY ALL THE CALLBACKS
	        // KEEP STATIC VALUES IN CONSTANTS
	        // KEEP DB INSTANCES AT ONEPLACE 
	        ...
	        db.open(function(err) {
	            ...
	            gfs.exist({ filename: req.params.filename, root: 'fs' }, function(err, found) {
	        ...
	        if (req.authenticationBool) {
	            ...
	            initFunctions.create_file_on_disk_to_extract_content(db, requested_Object, function(resultObject) {
	                if (resultObject.path && resultObject.path != "") {
	                    fs.unlinkSync(resultObject.path); //fs.unlinkSync(resultObject.path, function(){console.log('fs unlinkSync')});
	                }
	            ...
	            upload(req, res, function(err) {
	                if (err) {
	                    ...
	                    if (req.file.metadata && req.file.metadata.related_collection && req.file.metadata.related_collection == "job_applications") {
	                        setTimeout(function() {
	                            myObj["filename"] = req.file.filename
	    ...
	    // to get the image for the tinymce editor
	    app.get(backendDirectoryPath + '/image/:filename', function(req, res) {
	        /** First check if file exists */
	    ...
	    //download orders by processing pdf template saved in [template] table
	    app.get(backendDirectoryPath + '/orders/:id', requireLogin, function(req, res) {
	    ...
	    **/
	        ...
	        initFunctions.returnFindOneByMongoID(db, 'authentication_token', mongoIDField, function(result) {
	            if (result.aaData) {
	                ...
	                if (document.status == true) {
	                    ...
	                    db.collection('users').findAndModify({ _id: document.user_id }, [
	                        ['_id', 'asc']
	                    ], { $set: { "password": postJson.password } }, {}, function(err, result) {
	    ...
	    app.post(backendDirectoryPath + '/forgot_password', (req, res) => {
	        ...
	        initFunctions.crudOpertions(db, 'users', 'findOne', null, null, null, checkForExistence, function(result) {
	            if (result.aaData) {
	                var document = result.aaData,
	                    addAuthToken = new Object();
	                addAuthToken["user_id"] = document._id;
	                addAuthToken["status"] = true;


	                //create a secure token and send this token via email
	                initFunctions.create_crud_operation(db, 'authentication_token', 'create', addAuthToken, function(result) {
	    ...
	    app.get(backendDirectoryPath + '/task_scheduler', (req, res) => {
	        var schedulerFrom = req.query.schedulerFrom,
	            schedulerTo = req.query.schedulerTo,
	    ...
	    app.get(backendDirectoryPath + '/save_task_scheduler', (req, res) => {
	        ...
	        if (getJson.action == "create") {
	            ...
	            if (getJson.timeslip_id) {
	                ...
	                db.collection(table_nameStr).findAndModify({ _id: timeslipMongoID }, [
	                    ['_id', 'asc']
	                ], { $set: { "employee_id": getJson.emp_id, "timestamp_start": getJson.datetimestart, "timestamp_end": getJson.datetimeend } }, {}, function(err, result) {
	                    if (err) {
	        ...
	        if (req.authenticationBool) {
	            ...
	            if (postContent.team && postContent.team != "" && postContent.history_id && postContent.history_id != "") {
	                initFunctions.returnFindOneByMongoID(db, 'fixtures_history', postContent.history_id, function(resultObject) {
	                    if (resultObject.aaData) {
	                        var historyDetails = resultObject.aaData,
	                            teamPlayersArr = new Array();
	    ...
	    **/
	    ...
	    // akash get according to search 
	    app.get(backendDirectoryPath + '/search_navigator/', requireLogin, function(req, res) {
	        var collectionStr = "modules";
	        var outputObj = new Object();
	        if (req.authenticationBool) {
	            var loggedInUser = req.authenticatedUser;
	            returnUserAssignedSearchModules(loggedInUser._id, req, req.query.search, function(data) {
	                res.send(data);
	            });
	        } else {
	            outputObj["error"] = "Authorization error!";
	    ...
	    //save the navigation treeview click
	    // sidebar element click history
	    app.post(backendDirectoryPath + '/load_sidebar_action/', requireLogin, function(req, res) {
	        // console.log("inside here")
	        var collectionStr = req.body.collection;
	        var moduleIdStr = req.body.module_id;
	        var outputObj = new Object();
	        if (req.authenticationBool) {
	            initFunctions.save_sidebar_activity_log(db, collectionStr, moduleIdStr, req.authenticatedUser._id, req.authenticatedUser.active_system_uuid.toString(), function(result) {
	                // console.log("result",result)
	                res.send(result)
	            });
	        } else {
	    ...
	    // to fetch the status of sidebar categories from "sidebar_history"
	    app.post(backendDirectoryPath + '/fetch_sidebar_click_history', requireLogin, function(req, res) {
	        var collectionStr = req.body.collection
	            // console.log("inside",collectionStr)
	        var outputObj = new Object();
	        initFunctions.fetch_sidebar_activity_log(db, collectionStr, null, req.authenticatedUser._id, null, function(result) {
	    ...
	    **/
	    app.get(backendDirectoryPath + '/api_fetch_applications/', requireLogin, function(req, res) {
	        var itemsPerPage = 100,
	            pageNum = 1,
	            collectionStr = "job_applications",
	            outputObj = new Object();
	    ...
	    **/
	    app.get(backendDirectoryPath + '/api_fetch_list/', requireLogin, function(req, res) {
	        //console.log("In Fn: api_fetch_list")
	        var itemsPerPage = 100,
	            pageNum = 1,
	            templateStr = "",
	            collectionStr = "",
	        ...
	        if (req.authenticationBool) {
	        	...
	        	initFunctions.crudOpertions(db, 'system_preferences', 'findOne', null, "type", "default", "", function(result) {
		            if (result.aaData) {
		                outputObj["website_url"] = result.aaData.website_url;
		            }
		        });
		     //    db.collection("system_preferences").findOne({"type": "default"}).then((data)=> {
			    //      console.log("hdsfhsdhhsd",data)
			    //      outputObj["website_url"] = data.website_url
			    // }).catch(function (err) {//failure callback
			    //      console.log(err)
			    // });

			    // console.log("outputObj",outputObj)
	            //console.log("In Fn: api_fetch_list req is authenticated OK")
	            var activeSystemsStr = req.authenticatedUser.active_system_uuid.toString();
	            console.log("templateStr", templateStr)
	            console.log("activeSystemsStr", activeSystemsStr)
	            if (templateStr != "") {
	                //console.log("In Fn: api_fetch_list templateStr search is called")
	                ...
	                if (typeof activeSystemsStr !== 'undefined' && activeSystemsStr !== null && activeSystemsStr != "") {
	                    if (definedAdminTablesArr.indexOf(collectionStr) == -1) {
	                        if (collectionStr == "fs.files") {
	                            ...
	                            // newQueryObj['$or'] = [{ 'uuid_system': { $in: [activeSystemsStr] } },{"status": { $in: [ 1, "1" ] } }, { 'shared_systems': { $in: [activeSystemsStr] } }];
	                ...
	                console.log("collection find: ", newQueryObj);
	                ...
	                if (returnAllResults == "all") {
	                	...
	                	console.log("query1", newQueryObj)
	        ...
	        if (req.authenticationBool) {
	            if (collectionStr != "") {
	                init.MongoClient.connect(init.historyDatabaseURL, function(connErr, historyDB) {
	                    if (connErr) {
	                        outputObj["error"] = 'Unable to connect to the mongoDB server. ' + init.historyDatabaseURL;
	                        //cb(outputObj);
	                    } else {
	    ...
	    **/
	    app.get(backendDirectoryPath + '/api_fixture_history/', requireLogin, function(req, res) {
	        var itemsPerPage = 100,
	            pageNum = 1,
	            collectionStr = "fixtures_history",
	            selectedTeamUUIDStr = "",
	            total_records = 0;
	        var outputObj = new Object();
	                        ...
	                        if (result.aaData) {
	                            ...
	                            if (usersListArr.length > 0) {
	                                var userMongoID = new Array(),
	                                    useridAsString = new Array(),
	                                    playersAvailabilityArr = new Array();
	    ...
	    app.get(backendDirectoryPath + '/update_user_status', requireLogin, (req, res) => {
	        ...
	        if (req.authenticationBool) {
	            ...
	            if (search_id != "" && allow_web_access != "") {
	                db.collection('users').findAndModify({ _id: new mongodb.ObjectID(search_id) }, [
	                    ['_id', 'asc']
	                ], { $set: { "allow_web_access": parseInt(allow_web_access) } }, {}, function(err, result) {
	                    if (err) {
	                        ...
	                        if (resultData.events && resultData.events.length > 0) {
	                            ...
	                            if (resultData.events && resultData.events.length > 0) {
	                                ...
	                                for (var i = 0; i < matchesDetails.length; i++) {
	                                    var pushSubObjectBool = true,
	                                        currentRowTimestamp = matchesDetails[i].date_time;
	    ...
	    app.get(backendDirectoryPath + '/list/:id', requireLogin, system_preferences, function(req, res) {
	        if (req.authenticationBool) {
	            var pageRequested = req.params.id,
	                loggedInUser = req.authenticatedUser,
	            ...
	            returnUserAssignedModules(loggedInUser._id, req, function(allowedNavigationData) {
	                var assignedModuleBool = false,
	                    requestedPageStr = "/list/" + pageRequested,
	                    module_label_str = pageRequested.toUpperCase();
	        ...
	        if (req.authenticationBool) {
	            ...
	            if (req.query._id && req.query._id != "") {
	                ...
	                var queryString = req.url,
	                    removeUrl = backendDirectoryPath + '/' + req.params.id + '?';
	            ...
	            returnUserAssignedModules(req.authenticatedUser._id, req, function(allowedNavigationData) {
	                var assignedModuleBool = false,
	                    module_label_str = req.params.id;
	                if (allowedNavigationData && allowedNavigationData.modules && allowedNavigationData.modules.length > 0) {
	                    for (var i = 0; i < allowedNavigationData.modules.length; i++) {
	                        if (allowedNavigationData.modules[i].link == requestedPageStr) {
	                            assignedModuleBool = true;
	                ...
	                if (assignedModuleBool) {
	                    if (table_name == "") {
	                        ...
	                        if (typeof editFieldVal !== 'undefined' && editFieldVal !== null) {
	                            ...
	                            console.log("sdfsd",table_name,editFieldName,editFieldVal, queryStr)
	    ...
	    **/
	    ...
	    // to search the modules in search bar
	    function returnUserAssignedSearchModules(auth_user_id, req, search_str, cb) {
	        var outputObj = new Object();
	        if (auth_user_id != null && auth_user_id != 'undefined' && auth_user_id != "") {
	            //check user in groups
	            db.collection('groups').find({ "users_list": { $in: new Array(auth_user_id.toString()) }, "status": { $in: [1, "1"] } }).toArray(function(g_err, g_details) {
	                if (g_err) {
	                    ...
	                    var coll = db.collection('modules');
	                    var queryObj = { 'active': { $in: [1, '1'] }, $or: [{ 'name': new RegExp(search_str, 'i') }, { 'module_items': { $elemMatch: { 'label': new RegExp(search_str, 'i') } } }] }
