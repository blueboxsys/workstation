...
const mongoRemotePort = process.env.MONGO_REMOTE_PORT || 27017
const mongoRemoteDB = process.env.MONGO_REMOTE_DB || 'dev''
const mongoRemoteHost = process.env.MONGO_REMOTE_HOST || 'localhost'
const mongoRemoteURL = process.env.MONGO_REMOTE_URL || 'mongodb://${mongoRemoteHost}:${mongoRemotePort}/${mongoRemoteDB}'

let upload;
const mongo = {
    local: mongoURL,
    remote: mongoRemoteURL
}
...
describe('Testing -> GridFS Storage', () => {
    ...
    it('should throw an error when mongoose connection is not open', done => {
        try {
            const newUpload = multer({
                storage: new GridFsStorage({}, null)
            });
        } catch(error) {
            assert.ifError(error);
        }
