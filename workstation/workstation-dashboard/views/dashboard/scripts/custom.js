...
// fetch sidebar_history data
function fetchSidebarHistory(){
	return $.ajax({
        type: "POST",
        dataType: "json",
        async: false,
        url: backendDirectory+'/fetch_sidebar_click_history',
        data: {"collection": 'sidebar_history'},
        success: function(response) {
            if (response) {
               // console.log("response",response.aaData)
...
// match module_id with sidebar_history
function search_sidebar_label(idKey,SidebarClickData){
	if(SidebarClickData){
		for (var i=0; i < SidebarClickData.length; i++) {
	        if (SidebarClickData[i].module_id === idKey) {
	            // return SidebarClickData[i].state
	            return {"state":SidebarClickData[i].state,"time":"old"}
	        }
	    }
	}
    return {"state":"close","time":"new"}
    // return "close"
}
...
function load_navigation_data(){
	...
	if(search_str){
		var jsonRow = backendDirectory+'/search_navigator?search='+search_str;
	}else{
		var jsonRow = backendDirectory+'/load_navigator';
	}

	// to fetch the data of sidebar_history
	var SidebarClickData = fetchSidebarHistory().responseJSON['aaData']
	// console.log("SidebarClickData",SidebarClickData)
	...
	menuxhr=$.getJSON(jsonRow,function(result){
		if(result.aaData){
			...
			// console.log("openedFileNameStr",openedFileNameStr)
			var table_html='<li class="treeview';
			
			if(openedFileNameStr=="index" || openedFileNameStr=="" || openedFileNameStr=="/"){
				table_html+=' active ';
			}	
			table_html+='"><a href="'+backendDirectory+'/" target="_blank"><i class="fa fa-dashboard"></i> <span>Dashboard</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span><small>New</small></a></li>';
				...
				if(item.active==1){
					...
					if(item.module_items){
						if(item.module_items!=""){
							...
							var subTableHtmlStr="";	
							var category_status=search_sidebar_label(item._id,SidebarClickData)
							$.each(module_items, function(i,row){
								var linkStr=row.link;
								if(openedFileNameStr==row.link || category_status.state == 'open'){
									activeMenuFlag=true;
									subTableHtmlStr+='<li class="active">';				//open
								}else{
									subTableHtmlStr+='<li>';							//close
								}
								subTableHtmlStr+='<a href="'+backendDirectory+row.link+'" target="_blank" ';
								/*if(row.target==0){
									subTableHtmlStr+=' target="_blank" ';
								}*/
								// subTableHtmlStr+='><i class="fa fa-circle-o"></i> '+row.label+' <small>New</small></a></li>';
							...
							table_html+='<a href="#" id='+item._id+' onclick="change_navigation_click_state(this)">'+iconNameStr+' <span>'+item.name;

							if(category_status.time == 'new'){
								table_html+=' <small>New</small>';
							}
							table_html+='</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>';
							table_html+='<ul class="treeview-menu';

							if(activeMenuFlag){
								table_html += ' menu-open" style="display:block;'
							}
							table_html+='">'+subTableHtmlStr+'</ul>';
						}
					}   
				}             
			});
...
// <ul class="treeview-menu"></ul>   //close
// <ul class="treeview-menu menu-open" style="display: block;">   //open


// change the state of sidebar click state
var change_navigation_click_state = function(param)
{
    console.log(param.id)			//sidebar label
    // var sidebarjsonRow = backendDirectory+'/load_sidebar_action?label='+param.innerText;
    $.ajax({
        type: "POST",
        dataType: "json",
        url: backendDirectory + "/load_sidebar_action",
        data: { "collection": 'sidebar_history', "module_id": param.id },
        success: function(response) {
            if (response) {
		...
		$.getJSON(jsonRow,function(html){
			...
			if(html.aaData.length>0){
				$.each(html.aaData, function(i,row){
					...
					// 	contentHtml+="selected";
					// }
					if($("#selected_template").val()==row.code){
						contentHtml+="selected";
					}
