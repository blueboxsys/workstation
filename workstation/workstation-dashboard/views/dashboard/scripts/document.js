...
tinymce.init({
	...
	plugins: "advlist autolink link image lists charmap print preview hr anchor pagebreak searchreplace wordcount visualblocks visualchars code fullscreen insertdatetime media imagetools nonbreaking save table contextmenu directionality emoticons template paste textcolor",
	menubar: 'file edit view insert format tools table help',
  	toolbar: 'undo redo | bold italic underline strikethrough | fontselect fontsizeselect formatselect | alignleft aligncenter alignright alignjustify | outdent indent |  numlist bullist | forecolor backcolor removeformat | pagebreak | charmap emoticons | fullscreen  preview save print | insertfile image media code template link anchor codesample | ltr rtl',
	toolbar_sticky: true,
	...
	valid_elements: "*[*]",
	extended_valid_elements:"script[src|async|language|defer|type|charset]",
	...
	xhr.onload = function() {
		...
		// console.log("dsfds",xhr.responseText)
		...
		// return;
		// }
		success(window.location.origin+"/dashboard/image/"+json.filename);
	};
...
function load_template_dropdown(){
	$("#template").html("");
	var jsonRow=backendDirectory+"/api_fetch_list?limit=all&collection=templates";
	$.getJSON(jsonRow,function(response){
		...
		if(response.aaData.length>0){
			$.each(response.aaData, function(i,row){
				console.log("row",row)
				var tempcontentHtml="<option value='"+row.code+"' ";
...
// <% if (typeof contentObj.Type !== 'undefined' && contentObj.Type !== null && contentObj.Type == "page") { %>selected<% } %>
function load_systems(){
	$("#select_systems").html("");
	var jsonRow=backendDirectory+"/api_fetch_list?limit=all&collection=systems";
	$.getJSON(jsonRow,function(response){
		console.log("response",response)
		let website_url = response.website_url
		$("#hostname").val(website_url)
		if(website_url ==""){
			$(".viewPage").prop('disabled', true);
		}
		if(response.aaData){
		var contentHtml="";
		if(response.aaData.length>0){
